...
func toBFloat16(f float32) []byte {
	bs := make([]byte, 2)
	u32 := *(*uint32)(unsafe.Pointer(&f))
	binary.LittleEndian.PutUint16(bs, uint16(u32>>16))
	return bs
}
...
func main() {
	...
	schema := entity.NewSchema().WithName(collectionName).
		...
		WithField(entity.NewField().WithName(embeddingCol).WithDataType(entity.FieldTypeBFloat16Vector).WithDim(dim))
	...
	embeddingList := make([][]byte, 0, nEntities)
	...
	for i := 0; i < nEntities; i++ {
		vec := make([]byte, 0, dim*2)
		for j := 0; j < dim; j++ {
			vec = append(vec, toBFloat16(rand.Float32())...)
		}
		embeddingList = append(embeddingList, vec)
	}
	idColData := entity.NewColumnInt64(idCol, idList)
	randomColData := entity.NewColumnDouble(randomCol, randomList)
	embeddingColData := entity.NewColumnBFloat16Vector(embeddingCol, dim, embeddingList)
